...
// 进程监控事件处理
function updateProcessSort(sortBy) {
  processSortBy.value = sortBy
  fetchProcessInfo()
}

function updateProcessLimit(limit) {
...
<template>
  <div class="system-monitor-container p-4 bg-gray-50 h-screen overflow-hidden">
    ...
    <div class="grid grid-cols-4 gap-3 h-[calc(100vh-120px)]">
      ...
      <!-- 进程监控组件 -->
      <ProcessMonitor
        :process-info="processInfo" :process-sort-by="processSortBy" :process-limit="processLimit"
        :loading="loading" class="col-span-4" @update-sort="updateProcessSort" @update-limit="updateProcessLimit"
      />
    </div>
    ...
    >
      ...
      <div class="flex space-x-2 items-center">
        <button
          :disabled="loading" class="px-2 py-1 bg-blue-600 text-white rounded text-xs hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center"
          @click="refreshAllData"
        >
          <svg
            v-if="loading" class="animate-spin -ml-1 mr-1 h-3 w-3 text-white" xmlns="http://www.w3.org/2000/svg"
        ...
        <label class="flex items-center text-xs">
          <input v-model="autoRefresh" type="checkbox" class="mr-1" @change="toggleAutoRefresh">
