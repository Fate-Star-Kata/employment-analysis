...
import type { AdminArticleStatisticsResponse, AdminCreateArticlePayload, AdminKnowledgeArticlesQuery, AdminUpdateArticlePayload, KnowledgeArticle, KnowledgeCategory } from '@/types/factory'
import { ElMessage, ElMessageBox } from 'element-plus'
import { onMounted, reactive, ref } from 'vue'
import { adminBatchDeleteArticles, adminCreateArticle, adminDeleteArticle, adminGetArticleDetail, adminGetArticles, adminGetArticleStatistics, adminPublishArticle, adminUnpublishArticle, adminUpdateArticle } from '@/api/admin/knowledge'
...
// 方法
function handleSelectionChange(rows: KnowledgeArticle[]) { multipleSelection.value = rows }
...
function refreshAll() { getStats(); getList() }
...
// 创建
function openCreateDialog() { dialogs.create = true }
async function submitCreate(payload: AdminCreateArticlePayload) {
  loading.create = true
  try {
...
async function loadEditDetail(id: number) {
  loading.detail = true
  try {
...
async function submitEdit(id: number, payload: AdminUpdateArticlePayload) {
  loading.update = true
  try {
...
async function handlePublish(row: KnowledgeArticle) {
  try {
    ...
    if (res.code === 200) { ElMessage.success('发布成功'); getList() }
    else { ElMessage.error(res.msg || '发布失败') }
  }
  catch { /* 取消 */ }
}

async function handleUnpublish(row: KnowledgeArticle) {
  try {
    await ElMessageBox.confirm(`确定取消发布《${row.title}》吗？`, '取消发布确认', { type: 'warning' })
    const res = await adminUnpublishArticle(row.id)
    if (res.code === 200) { ElMessage.success('已取消发布'); getList() }
...
// 删除
function confirmDelete(row: KnowledgeArticle) {
...
function confirmBatchDelete() {
  ...
  ElMessageBox.confirm(`确定批量删除选中的 ${ids.length} 篇文章吗？不可恢复。`, '批量删除确认', { type: 'warning' })
    .then(async () => {
      ...
      if (res.code === 200) { ElMessage.success('批量删除成功'); getList() }
