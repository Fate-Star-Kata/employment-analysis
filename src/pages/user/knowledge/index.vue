...
import type { KnowledgeArticle, KnowledgeArticleDetail, KnowledgeCategory } from '@/types/factory'
import { marked } from 'marked'
import { Motion } from 'motion-v'
import { computed, defineComponent, h, onBeforeUnmount, onMounted, ref } from 'vue'
import { getKnowledgeArticleDetail, getKnowledgeArticles, getKnowledgeCategories, likeKnowledgeArticle } from '@/api/user/knowledge'
          ...
          <div class="flex items-center justify-center gap-3 pt-2">
            <div class="form-control w-full max-w-md">
              <div class="join w-full">
                <input
                  v-model="searchQuery" type="text" placeholder="搜索文章..."
                  class="input input-bordered join-item flex-1" @input="handleSearch"
                >
      ...
      <div class="max-w-6xl mx-auto px-4">
        <RevealMotion :delay="0">
          <div class="flex flex-wrap justify-center gap-2">
            <button
              v-for="category in categories" :key="category" class="btn btn-sm rounded-full"
              :class="selectedCategory === category ? 'btn-primary' : 'btn-outline'" :disabled="loading"
            ...
            >
              <figure class="px-6 pt-6">
                <div class="w-full h-48 rounded-xl overflow-hidden">
                  ...
                  >
                  <div
                    class="w-full h-full bg-gradient-to-br from-primary/20 to-secondary/20 flex items-center justify-center" :class="[
                      getImageUrl(article.image) ? 'hidden' : 'flex',
                    ]"
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg" class="h-16 w-16 opacity-50" fill="none" viewBox="0 0 24 24"
                      stroke="currentColor"
                    >
                      <path
                        stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
              ...
              <div class="card-body">
                ...
                <p class="opacity-70 text-sm line-clamp-3">
                  {{ article.summary }}
                </p>
                <div class="card-actions justify-between items-center mt-4">
                  ...
                  <div class="flex items-center gap-2">
                    <button class="btn btn-ghost btn-xs" @click.stop="toggleLike(article.id)">
        ...
        <div v-if="!loading && filteredArticles.length === 0" class="text-center py-16">
          <RevealMotion :delay="0">
            <div class="opacity-50">
              <svg
                xmlns="http://www.w3.org/2000/svg" class="h-16 w-16 mx-auto mb-4" fill="none" viewBox="0 0 24 24"
                stroke="currentColor"
              >
                <path
                  stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
              ...
              <p class="text-sm">
                {{ searchQuery ? '没有找到相关文章，请尝试其他关键词' : '该分类下暂无文章' }}
              </p>
            </div>
          </RevealMotion>
        </div>
