...
async function getCaptchaConfig() {
  try {
    ...
    if (res.code === 200 && res.data) {
      ...
      // 赋值给表单，确保数据类型正确
      captchaForm.CAPTCHA_LENGTH = configMap.CAPTCHA_LENGTH ? Number.parseInt(configMap.CAPTCHA_LENGTH) : 4
      captchaForm.CAPTCHA_TIMEOUT = configMap.CAPTCHA_TIMEOUT ? Number.parseInt(configMap.CAPTCHA_TIMEOUT) * 60 : 300 // 转换为秒
      captchaForm.CAPTCHA_FONT_SIZE = configMap.CAPTCHA_FONT_SIZE ? Number.parseInt(configMap.CAPTCHA_FONT_SIZE) : 30
      ...
      if (configMap.CAPTCHA_IMAGE_SIZE) {
        try {
          ...
          const [width, height] = sizeStr.split(',').map((s: string) => Number.parseInt(s.trim()))
...
<template>
  ...
  >
    <el-form
      ref="formRef" v-loading="loading" :model="captchaForm" :rules="rules" label-width="140px"
      class="config-form"
    >
      <el-row :gutter="24">
        <el-col :span="8">
          ...
          >
            <el-form-item label="验证码长度" prop="CAPTCHA_LENGTH">
              <el-input-number
                v-model="captchaForm.CAPTCHA_LENGTH" :min="3" :max="8" placeholder="请输入验证码长度"
                class="w-full"
              />
        ...
        <el-col :span="8">
          ...
          >
            <el-form-item label="超时时间(秒)" prop="CAPTCHA_TIMEOUT">
              <el-input-number
                v-model="captchaForm.CAPTCHA_TIMEOUT" :min="60" :max="3600" placeholder="请输入超时时间"
                class="w-full"
              />
              <el-text class="block mt-1 text-gray-500" size="small">
                验证码有效期
        ...
        <el-col :span="8">
          ...
          >
            <el-form-item label="字体大小" prop="CAPTCHA_FONT_SIZE">
              <el-input-number
                v-model="captchaForm.CAPTCHA_FONT_SIZE" :min="12" :max="50" placeholder="请输入字体大小"
                class="w-full"
              />
              <el-text class="block mt-1 text-gray-500" size="small">
                字体大小（像素）
      ...
      >
        <el-form-item label="图片尺寸">
          <el-radio-group v-model="captchaForm.CAPTCHA_IMAGE_SIZE" class="image-size-group">
            <el-radio
              v-for="option in imageSizeOptions" :key="option.label" :value="option.value"
          ...
          <el-card class="preview-card" shadow="never">
            <div class="preview-content">
              <div class="preview-item">
                <el-text class="preview-label">
                  验证码长度：
                </el-text>
                <el-tag type="primary">
                  {{ captchaForm.CAPTCHA_LENGTH }} 位
                </el-tag>
              </div>
              <div class="preview-item">
                <el-text class="preview-label">
                ...
                <el-tag type="success">
                  {{ Math.floor(captchaForm.CAPTCHA_TIMEOUT / 60) }} 分钟
                </el-tag>
              </div>
              <div class="preview-item">
                <el-text class="preview-label">
                ...
                <el-tag type="info">
                  {{ captchaForm.CAPTCHA_IMAGE_SIZE[0] }}x{{ captchaForm.CAPTCHA_IMAGE_SIZE[1]
                  }}
                </el-tag>
              </div>
              <div class="preview-item">
                ...
                <el-tag type="warning">
                  {{ captchaForm.CAPTCHA_FONT_SIZE }}px
                </el-tag>
              </div>
            </div>
            <el-divider />
